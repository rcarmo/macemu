...
**Status**: ✅ 251/251 interp-vs-production-JIT equivalence (includes `shift_ra_upper_clean` for RA-cache upper-32 canonicalisation after 64-bit host shifts, fixed-count `lswi`/`stswi` multi-access coherency vectors, `mfmsr_basic`, AltiVec exclusion vectors for wrong-function/pass-through handlers, `fcmpu_nan_unordered` for FP unordered/FPSCR compare semantics, `fp_fctiw_nearest` for FPSCR rounding, and `vec_vscr_roundtrip` for architectural VSCR state).
...
L3  Lazy CR0/register allocation revalidated           (all harnesses green + boot proof) — ACHIEVED:
    ...
    barrier; harness 251/251 + strict-JIT desktop holds
