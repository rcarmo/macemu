...
| 5 | Interpreter and JIT builds agree on shared semantics | ✅ 244/244 interp-vs-production-JIT opcode equivalence (the harness compares interpreter mode against the real JIT dispatch loop; 2026-06-22 this replaced a JIT-vs-JIT determinism check and surfaced+fixed nand/addme/subfme/divw codegen bugs; later added `shift_ra_upper_clean` for RA-cache upper-32 canonicalisation, fixed-count `lswi`/`stswi` vectors for multi-access coherency, and `mfmsr_basic` for the interpreter's simplified MSR value). `bcl` LR update fixed (2026-05). |
