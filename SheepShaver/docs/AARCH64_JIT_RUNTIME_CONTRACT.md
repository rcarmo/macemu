...
| 3 | Helper calls are semantic barriers | ✅ Guarded load/store, dcbz, timebase, and lwarx/stwcx helpers are localized H2 calls (callee-saved x19–x29 + RA barrier / no guest-state mutation keep the struct coherent across re-entry). EMUL_OP and unhandled ops remain full block barriers via interpreter delegation. |
