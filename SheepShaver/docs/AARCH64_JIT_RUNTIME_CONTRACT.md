...
- `sheepshaver_jit_dcbz` — exact `dcbz` memory-layer helper. Generated code computes EA, then calls
  `Mac_memset(ea & ~31, 0, 32)`, matching interpreter `vm_memset` semantics instead of raw host stores.
- `sheepshaver_jit_safe_load_reversed` / `sheepshaver_jit_safe_store_reversed` — exact byte-reversed
  integer memory helpers for `lhbrx/sthbrx/lwbrx/stwbrx`, using the same size-aware validity/page-mapped
  checks as scalar helpers and `vm_read/write_memory_{2,4}_reversed` for interpreter-equivalent byte order.
- `sheepshaver_jit_fp_load` / `sheepshaver_jit_fp_store` — exact FP memory helpers for `lfs/lfd/stfs/stfd`
...
- `sheepshaver_jit_lmw/stmw/lswi/stswi` — helper-routed fixed-count string/multiple memory operations
  using existing safe load/store primitives. Call sites flush dirty cached GPRs before the helper and
  reset RA afterwards because these helpers read/write the architectural register struct directly.
  Runtime-count `lswx/stswx` remain on their proven inline generators; a C-helper replacement was
  rejected after passing opcode equivalence but causing strict-desktop early exit.
...
| 5 | Interpreter and JIT builds agree on shared semantics | ✅ 285/285 interp-vs-production-JIT opcode equivalence (the harness compares interpreter mode against the real JIT dispatch loop; 2026-06-22 this replaced a JIT-vs-JIT determinism check and surfaced+fixed nand/addme/subfme/divw codegen bugs; later added RA-width, string/multiple, SPR/FPSCR/AltiVec, addis/lis, vsel, AltiVec FP compare, vperm control-mask, bcctr CTR-decrement, fres/vector-estimate delegation, FPSCR move/write delegation, FP Rc/FPSCR-producing-op delegation, AltiVec vector-sum delegation, AltiVec average/saturating add-sub delegation, AltiVec merge/multiply/conversion/shift delegation, AltiVec signed/pixel unpack and pack delegation, PPC64 Rc delegation, and CR6 dotted-vector coverage). `bcl` LR update fixed (2026-05). |
