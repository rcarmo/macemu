...
| 5 | Interpreter and JIT builds agree on shared semantics | ✅ 251/251 interp-vs-production-JIT opcode equivalence (the harness compares interpreter mode against the real JIT dispatch loop; 2026-06-22 this replaced a JIT-vs-JIT determinism check and surfaced+fixed nand/addme/subfme/divw codegen bugs; later added `shift_ra_upper_clean` for RA-cache upper-32 canonicalisation, fixed-count `lswi`/`stswi` vectors for multi-access coherency, `mfmsr_basic`, AltiVec exclusion vectors for wrong-function/pass-through handlers, `fcmpu_nan_unordered`, `fp_fctiw_nearest`, and `vec_vscr_roundtrip`). `bcl` LR update fixed (2026-05). |
