...
**2026-06-13 ARM64 JIT performance lane:** Full-JIT correctness is now strict-clean and default (`JIT-STATUS.md`, commit `980a0451`). The 2×/CPU-load target is a measured optimization tranche, not a correctness bring-up tranche. First experiments showed 32768KB translation-cache churn (18 hard flushes in a 20s ROM/headless profile window versus 2 with 131072KB), so the default full-JIT harness/QA cache has been raised to 131072KB. Apparently low-risk runtime-output and stable-edge direct-chain changes can alter timing enough to expose strict marker regressions, so they remain investigation items rather than defaults. Follow-up profiling found a larger safe win: zero-filled RAM probes were being traced and compiled repeatedly; the AArch64 dispatcher now interprets those zero-RAM chunks directly with a MAXRUN cap instead of generating throwaway optlev0 blocks. In a 20s diagnostic ROM window this reduced compile calls from ~6.4M to ~79k and hard flushes from 5 to 1 while preserving strict markers. A second safe micro-optimization replaced only the exact generated `OR.B #<data>,D0` interpreter handler (`cpufunctbl[0]`) with an AArch64 fast path; this preserves the interpreter cadence but avoids `get_ibyte()`/`get_real_address()` overhead in the dominant zero-probe fallback. The comparable 20s perfstat window improved guest progress from ~28.3M to ~50.2M dispatch-counter units with similar task-clock, i.e. about 1.77× more guest work per CPU second after the zero-RAM compile skip. The fast `OR.B #<data>,D0` path now also computes byte-test flags directly and skips the D0 write-back for the common zero-immediate case, and disabled trace/probe hooks avoid unnecessary `m68k_getpc()` calls on hot interpreter/tracing loops. Disabled write-trace hooks are also now guarded before the put helpers call into trace formatting; this removed `basilisk_trace_host_write(...)` / `trace_write_log(...)` from the sampled hot list in the 20s ROM profile. The trace-build hot path also no longer clears the full `pc_hist[MAXRUN]` array on non-UAE ARM64 builds, since the compiler consumes only the populated `location` entries; a 20s perfstat window kept strict markers clean while task-clock dropped to ~22.7s and guest progress stayed around ~50.0M dispatch-counter units. Disabled JIT diagnostics are now kept out of the dispatcher hot path by guarding their counters and print hooks behind `B2_JIT_DIAG`, leaving diagnostics available when explicitly enabled without taxing normal ROM/profile runs. The fast interpreter override is now installed once during compiler-table setup rather than re-checking the install guard on every `execute_normal()` / `exec_nostats()` entry. The trace loop also hoists the `B2_JIT_MAXRUN` environment/cache lookup out of the per-opcode loop, preserving `B2_JIT_MAXRUN` semantics while avoiding a repeated static-guard branch during block tracing. A broader batched zero-probe shortcut, direct opcode fetch, inline `op_0_0` dispatch, and a fast `(An)` immediate-OR handler were rejected because they either entered the ROM exception-frame spin earlier, slowed boot progress, or broke the opcode harness. Keep strict marker checks (`JIT_FALLBACK`, `SEGV_SKIP`, `JITBLOCKVERIFY`, `op=8c4c`, `bad_pcp`, and fatal host signal markers) mandatory after every speed change; desktop CPU-load claims still require real VNC/display evidence.
