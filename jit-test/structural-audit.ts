...
const memoryEmitterReportSource = await Bun.file(new URL(
...
const memoryEmitterProbeSource = await Bun.file(new URL(
  "./emitter-memory-conformance.cpp",
  import.meta.url,
)).text();
...
const fsccMatrixSource = await Bun.file(new URL(
...
const fsccGeneratorStart = gencompSource.indexOf("     case i_FScc:");
const fsccGeneratorEnd = gencompSource.indexOf("     case i_FTRAPcc:", fsccGeneratorStart);
if (fsccGeneratorStart < 0 || fsccGeneratorEnd < 0) fail("configured FScc generator route is incomplete");
const fsccGeneratorBody = gencompSource.slice(fsccGeneratorStart, fsccGeneratorEnd);
for (const contract of ["uses_fpu;", "mayfail;", "uses_cmov;", "comp_fscc_opp(opcode,extra);", "failure = 1;"])
  requireText(fsccGeneratorBody, contract, "configured FScc generator route");
...
if (fsccMidStart < 0 || fsccMidEnd < 0) fail("missing live FScc MIDFUNC fp_fscc_ri");
requireText(midfuncSource.slice(fsccMidStart, fsccMidEnd), "raw_fp_fscc_ri(d, cc);", "live FScc MIDFUNC fp_fscc_ri");
...
if (fsccRawStart < 0 || fsccRawEnd < 0) fail("missing live FScc raw boundary raw_fp_fscc_ri");
const fsccRawBody = codegenSource.slice(fsccRawStart, fsccRawEnd);
for (const contract of [
  "switch (cc)", "case NATIVE_CC_F_F:", "case NATIVE_CC_F_EQ:",
  ...
  "case NATIVE_CC_F_NE:", "case NATIVE_CC_F_T:", 'jit_abort("invalid AArch64 FScc condition")',
]) requireText(fsccRawBody, contract, "live FScc raw condition table");
const fsccRawCounts = [
  ["case NATIVE_CC_", 16], ["CLEAR_LOW8_xx(", 11], ["SET_LOW8_xx(", 11],
...
for (const [token, expected] of fsccRawCounts) {
  ...
  if (count !== expected) fail(`live FScc raw ${token} count=${count}, expected=${expected}`);
}
...
for (const contract of [
  "if (extra & 0x20)", "if ((opcode & 0x38) != 0)", "preserve_flags_before_nzcv_clobber();",
  "fflags_into_flags();", "fp_fscc_ri(reg, native_fp_cc_base + (extra & 0x0f));", "discard_flags_in_nzcv();",
]) requireText(liveFsccBody, contract, "configured FScc compiler route");
for (const contract of [
  "for (let cc = 0; cc < 32; cc++)", "for (const reg of [0, 7])", 'sr === "271f"',
  'fpsr === state.fpsr', 'name: "invalid_cc_20"', 'name: "memory_aind_strict"',
  'name: "memory_aind_false"', 'name: "memory_postinc_true_alias"',
  'name: "memory_predec_unordered"', 'name: "memory_d16_equal"',
  'strict full-JIT: opcode fallback', "expectedTotal = process.env.CASE ? 1 : 326",
]) requireText(fsccMatrixSource, contract, "FScc lifecycle matrix");
...
const fsccLow8Corpus = `${codegenSource}\n${midfuncSource}\n${midfunc2Source}\n${compatSource}\n${generatedSource}`;
const fsccGlobalClearSites = (fsccLow8Corpus.match(/\bCLEAR_LOW8_xx\(/g) || []).length;
const fsccGlobalSetSites = (fsccLow8Corpus.match(/\bSET_LOW8_xx\(/g) || []).length;
if (fsccGlobalClearSites !== 13 || fsccGlobalSetSites !== 11)
  fail(`FScc low-byte emitter global sites clear/set=${fsccGlobalClearSites}/${fsccGlobalSetSites}, expected 13/11`);
console.log("METRIC structural_fscc_generator_lifecycle_audited=1");
console.log("METRIC structural_fscc_native_predicates=32");
console.log("METRIC structural_fscc_native_runtime_cases=320");
console.log("METRIC structural_fscc_memory_service_cases=4");
console.log("METRIC structural_fscc_negative_cases=2");
console.log("METRIC structural_fscc_live_midfunc_raw_boundaries=2");
console.log("METRIC structural_fscc_live_low8_emitters=2");
...
/* Direct per-block checksum entry: load the canonical host PC, branch to the
 * set-PC popall label, publish the complete PC triple, unwind, and tail-enter
 * checksum validation. Runtime test instrumentation is env-gated and must not
 * alter ordinary dispatch policy. */
const rawChecksumBody = bodyBetween(
  "LOWFUNC(NONE,WRITE,1,compemu_raw_check_checksum,(MEMR s))\n{",
  "LENDFUNC(NONE,WRITE,1,compemu_raw_check_checksum",
);
for (const contract of [
  "LOAD_U64(REG_WORK1, s);", "LDR_xXi(REG_WORK1, REG_WORK1, 0);",
  "uae_u32* branchadd = (uae_u32*)get_target();", "B_i(0);",
...
const checksumPopall = functionBody(
  ...
  "raw checksum popall boundary",
);
for (const contract of [
  "compemu_raw_set_pc_from_reg(REG_WORK1);", "raw_pop_preserved_regs();",
  "compemu_raw_jmp((uintptr)check_checksum);", "jit_test_direct_checksum_entries",
...
/* Per-block first-entry dispatcher stub: load the block-owned host-PC pointer,
 * publish canonical PC through the direct side of the shared popall boundary,
 * unwind, and tail-enter execute_normal. The runtime hook must first use the
 * ordinary BI_NEED_RECOMP transition and may only override its owned cache
 * line; execute_normal_cycles retains its separate countdown contract. */
const rawExecuteNormalBody = bodyBetween(
  "LOWFUNC(NONE,WRITE,1,compemu_raw_execute_normal,(MEMR s))\n{",
  "LENDFUNC(NONE,WRITE,1,compemu_raw_execute_normal",
);
for (const contract of [
  "LOAD_U64(REG_WORK1, s);", "LDR_xXi(REG_WORK1, REG_WORK1, 0);",
  "uae_u32* branchadd = (uae_u32*)get_target();", "B_i(0);",
  "write_jmp_target(branchadd, (uintptr)popall_execute_normal_setpc);",
]) requireText(rawExecuteNormalBody, contract, "raw execute_normal body");
requireBefore(rawExecuteNormalBody, "LOAD_U64(REG_WORK1, s);", "LDR_xXi(REG_WORK1, REG_WORK1, 0);", "raw execute_normal pointer load");
requireBefore(rawExecuteNormalBody, "LDR_xXi(REG_WORK1, REG_WORK1, 0);", "B_i(0);", "raw execute_normal PC before branch");
const executeNormalPopall = functionBody(
  allocatorSource,
  "popall_execute_normal_setpc = get_target();",
  "popall_check_checksum_setpc = get_target();",
  "raw execute_normal popall boundary",
);
for (const contract of [
  "jit_test_direct_execute_normal_entries", "compemu_raw_set_pc_from_reg(REG_WORK1);",
  "popall_execute_normal = get_target();", "raw_pop_preserved_regs();",
  "compemu_raw_jmp((uintptr)execute_normal);",
]) requireText(executeNormalPopall, contract, "raw execute_normal popall contract");
requireBefore(executeNormalPopall, "jit_test_direct_execute_normal_entries", "popall_execute_normal = get_target();", "raw execute_normal counter on direct side");
requireBefore(executeNormalPopall, "compemu_raw_set_pc_from_reg(REG_WORK1);", "raw_pop_preserved_regs();", "raw execute_normal PC before unwind");
requireBefore(executeNormalPopall, "raw_pop_preserved_regs();", "compemu_raw_jmp((uintptr)execute_normal);", "raw execute_normal tail ABI");
const rawExecuteNormalTestHook = functionBody(
  allocatorSource,
  "bool jit_test_prepare_direct_execute_normal_entry(void)",
  "STATIC_INLINE void create_popalls(void)",
  "raw execute_normal runtime hook",
);
for (const contract of [
  "B2_TEST_FORCE_DIRECT_EXECUTE_NORMAL", "bi->direct_pen", "bi->status != BI_ACTIVE",
  "const uae_u32 cl = cacheline(bi->pc_p);", "if (bi != cache_tags[cl + 1].bi)",
  "block_need_recompile(bi);", "cache_tags[cl].handler = bi->direct_pen;",
]) requireText(rawExecuteNormalTestHook, contract, "raw execute_normal runtime contract");
requireBefore(rawExecuteNormalTestHook, "block_need_recompile(bi);", "cache_tags[cl].handler = bi->direct_pen;", "raw execute_normal lifecycle before override");
for (const contract of [
  "jit_test_prepare_direct_execute_normal_entry()",
  "B2_TEST_FORCE_DIRECT_EXECUTE_NORMAL could not prepare direct execute_normal entry",
...
for (const [name, report] of [
  ...
  const row = closureInventoryCsv.split("\n").find((line) => line.startsWith(`emitter_api,${name},`));
  const expectedStatus = report ? "audited" : "unreviewed";
  if (!row?.includes(`,${expectedStatus},`) || (report && !row.includes(report)))
    fail(`raw inc-opcount child emitter ${name} independent classification changed`);
}
...
const publishedMemoryEmitterNames = [
  "LDP_xxXi", "LDP_xxXpost", "LDR_wXi", "LDR_wXx", "LDR_wXxLSLi", "LDR_xXi",
  "LDR_xXpost", "LDR_xXxLSLi", "LDRB_wXx", "LDRH_wXi", "LDRH_wXx", "STP_wwXi",
  "STP_xxXi", "STP_xxXpre", "STR_wXi", "STR_wXx", "STR_wXxLSLi", "STR_xXi",
  "STR_xXpre", "STRB_wXx", "STRH_wXi", "STRH_wXx",
];
const restorePublishedMemoryEmitterRows = (csv: string) => {
  let restored = csv;
  for (const name of publishedMemoryEmitterNames) {
    const current = restored.split("\n").find((line) => line.startsWith(`emitter_api,${name},`));
    if (!current) fail(`integer memory emitter row missing during predecessor reconstruction: ${name}`);
...
/* Integer memory encoders share scaling, writeback, lane, pair-order and NZCV
 * contracts. Direct guest-memory forms intentionally encode natmem as Rn and
 * zero-extend the 32-bit guest offset as Rm through UXTW. */
const memoryEmitterNames = [
  "LDP_xxXi", "LDP_xxXpost", "LDR_wXi", "LDR_wXx", "LDR_wXxLSLi", "LDR_xXi",
  "LDR_xXpost", "LDR_xXxLSLi", "LDRB_wXx", "LDRH_wXi", "LDRH_wXx", "STP_wwXi",
  "STP_xxXi", "STP_xxXpre", "STR_wXi", "STR_wXx", "STR_wXxLSLi", "STR_xXi",
  "STR_xXpre", "STRB_wXx", "STRH_wXi", "STRH_wXx",
...
for (let i = 0; i < memoryEmitterNames.length; ++i) {
  ...
  const found = (memoryEmitterCallers.match(new RegExp(`\\b${name}\\(`, "g")) || []).length;
  if (found !== memoryRawCallers[i]) fail(`generic ${name} raw caller census: expected ${memoryRawCallers[i]}, found ${found}`);
  const row = closureInventoryCsv.split("\n").find((line) => line.startsWith(`emitter_api,${name},`));
  if (!row?.includes(",audited,") || !row.includes(`,${memoryInventoryReferences[i]},`) ||
      !row.includes("AARCH64_JIT_AUDIT_INTEGER_MEMORY_EMITTERS.md"))
    fail(`generic ${name} configured census or closure promotion changed`);
}
for (const contract of [
  "Use UXTW to zero-extend 32-bit M68k addresses to 64-bit host offsets",
  "#define LDR_wXx(Wt,Xn,Xm)", "#define LDRB_wXx(Wt,Xn,Xm)", "#define LDRH_wXx(Wt,Xn,Xm)",
  "#define STR_wXx(Wt,Xn,Xm)", "#define STRB_wXx(Wt,Xn,Xm)", "#define STRH_wXx(Wt,Xn,Xm)",
]) requireText(codegenHeaderSource, contract, "generic integer memory UXTW encoding");
for (const contract of [
  "0xa9602969u", "0xa8dfb5ccu", "0xb97ffd49u", "0xb86d498bu", "0xb87079eeu",
  "0xf97ffd49u", "0xf850058bu", "0xf86f79cdu", "0x386b4949u", "0x797ffdacu",
  "0x787049eeu", "0x29202969u", "0xa91fb5ccu", "0xa9a0422fu", "0xb93ffd49u",
  "0xb82d498bu", "0xb83079eeu", "0xf93ffd49u", "0xf8100d8bu", "0x382f49cdu",
  "0x793ffe30u", "0x78344a72u", "MAP_NORESERVE", "0x80000000ull",
  "LDRB UXTW high bit", "LDRH UXTW high bit", "LDR W UXTW high bit",
  "STRB UXTW high bit", "STRH UXTW high bit", "STR W UXTW high bit",
  "memory op preserves NZCV", "exact_words==22&&native_values==31&&nzcv_vectors==22&&uxtw_vectors==6",
]) requireText(memoryEmitterProbeSource, contract, "generic integer memory native conformance");
for (const contract of ["-Wall -Wextra -Werror", "emitter-memory-conformance.cpp"])
...
if (/Review state: \*\*pending independent review\*\*|Review state: \*\*pending final (?:re-review|readiness review)\*\*/i.test(memoryEmitterReportSource))
  fail("generic integer memory evidence report is not final");
for (const contract of [
  "**22/22** canonical boundary instruction words", "**31/31** native value", "**22/22** NZCV-preservation",
  "**6/6** direct byte/halfword/word", "configured closure\nreference total is **168**",
  "contains **170**\nraw calls", "`147 audited / 44 unreviewed`",
  "final re-review: **approve**",
]) requireText(memoryEmitterReportSource, contract, "generic integer memory final evidence");
console.log("METRIC structural_memory_emitter_apis=22");
console.log("METRIC structural_memory_emitter_exact_words=22");
