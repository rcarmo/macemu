...
try {
  ...
  for (const item of selectedService) {
    ...
    try {
      ...
      else { fail++; console.error(`FPP_SGLMUL_FAIL case=${item.name} rc=${run.status} mem=${mem} want=${expected} d0=${d0} want_d0=${expectedD0} fpsr=${fpsr} want_fpsr=${item.fpsr} a0=${a0} want_a0=${expectedA0} sr=${sr} fallback_profile=${profile} want_profile=${expectedProfile}`); }
