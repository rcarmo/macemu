...
if phase_enabled emitters; then
    ...
    timeout -k 5s 60s "$SCRIPT_DIR/emitter-ands-conformance.sh" || emit_failure_metrics 1 "ARM64 AND-with-flags emitter conformance failed" 0
    timeout -k 5s 60s "$SCRIPT_DIR/emitter-asr-conformance.sh" || emit_failure_metrics 1 "ARM64 ASR emitter conformance failed" 0
    ...
    timeout -k 5s 180s "$SCRIPT_DIR/raw-metadata-rmw-boundary-matrix.sh" || emit_failure_metrics 1 "ARM64 raw metadata RMW boundary conformance failed" 0
    timeout -k 5s 120s "$SCRIPT_DIR/raw-exec-nostats-boundary-matrix.sh" || emit_failure_metrics 1 "ARM64 raw exec_nostats boundary conformance failed" 0
    timeout -k 5s 120s "$SCRIPT_DIR/raw-execute-normal-boundary-matrix.sh" || emit_failure_metrics 1 "ARM64 raw execute_normal boundary conformance failed" 0
    ...
    timeout -k 5s 120s "$SCRIPT_DIR/raw-maybe-do-nothing-conformance.sh" || emit_failure_metrics 1 "ARM64 raw maybe-do-nothing exact-native conformance failed" 0
    timeout -k 5s 180s "$SCRIPT_DIR/raw-maybe-do-nothing-boundary-matrix.sh" || emit_failure_metrics 1 "ARM64 raw maybe-do-nothing live boundary conformance failed" 0
    timeout -k 5s 180s "$SCRIPT_DIR/raw-mov-host-memory-conformance.sh" || emit_failure_metrics 1 "ARM64 raw integer host-memory move conformance failed" 0
