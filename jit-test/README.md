...
`jit-test/regalloc-pressure.sh` supplies 20 non-baseline allocator controls
...
operations, CMPM/CMPA, ADD, and AND. The ADD cells separately force a fetched
source toward its architectural destination and FLAGX toward the private
pre-write EA. The AND cells force the same source-first ownership and the
shared logical destination EA toward its arithmetic source. This distinguishes
MIDFUNC operand ownership from generator-level EA pins rather than assuming
that a register-rich baseline pass exercises either collision.
