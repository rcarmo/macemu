...
**JIT optlev=2:** ⚠️ Full-JIT default. The historical `040ba0xx` late-ROM spin is **FIXED (2026-06-22)** — see below. With three fixes (lazy-cache-flush staleness + cache-tag aliasing + dispatcher-diagnostic gating) the all-native boot now clears the NuBus/Slot-Manager frontier, the `040026f8` aliasing frontier, and the `0402e8d0` slot-ROM bfextu loop, advancing through `InitAll` to **video driver init** (`VideoDriverOpen` → `SetEntries` palette → `SetGamma`, gray-screen stage); desktop not yet reached. Current frontier: post-video-init NuBus probe at guest `0408795a` (`mac=f0000000`, handled by SEGV_SKIP).
...
The `040ba0xx` spin was **NOT** a resource-map `d0` corruption (that earlier hypothesis was disproven 2026-06-22 by interp ground-truth: the `0x0401beb4` offset-shift loop matches the interpreter exactly). The real cause: **lazy translation-cache invalidation (`jitlazyflush=true`) reuses STALE compiled blocks.** Early-boot driver/Slot-Manager init reuses RAM buffers across `_BlockMove`/`FlushCodeCache` cache flushes; the lazy soft-flush (mark `BI_NEED_CHECK` + checksum-reactivate) wrongly reuses an out-of-range stale block (old driver code) → a different driver list (`a1=0000cdc0` vs interp `0000bd00`) → the JIT walks the legit slot declaration ROM (`LowMem 0x824`=`0x05010000`) into the no-exit NuBus slot scanner (`040023xx`→`0400246e`→`040b9874`→the `040b98f2`↔`040ba0xx` SCC poll) — which the interpreter never enters. **Fix:** `jitlazyflush` default → `false` (eager `flush_icache_hard`), forcing recompile so no stale block runs; boot clears NuBus. Verified: NuBus spin gone; jit-test `fail_equiv=0` (no correctness regression). Both modes are 100% JIT (no interpreter fallback). Full campaign analysis (CONT.58–73) in `notes/macemu-jit-io-rootcause.md`. **Subsequent fix (6596f558):** the `040026f8` frontier was cache-tag aliasing (TAGMASK `0xffff` too small for 64MB RAM → distant guest PCs collide in `cache_tags` → wrong block via unchecked direct-linked jumps); enlarged TAGMASK → `0x3ffff` (4x tags). **Third fix (fd4c48fe):** the `0402e8d0` frontier was NOT a real codegen bug — it was an **observation artifact**. The JIT dispatch loop carried an unconditional per-dispatch `JIT_ENTRY` fprintf + a large DC diagnostic block; the per-dispatch `fprintf`+`fflush` perturbed dispatch timing enough to spin a slot-ROM `bfextu` blit/expand loop at `0402e8d0`. Controlled test (identical canonical config, only the flag differs): `B2_JIT_DIAG=1` spins forever at `0402e8d0` (DC 970M, pc pinned); default-off completes the bfextu loop (`d5` 4→0, `d6` -16→112) and advances to video init. Per-call trace proved the bfextu handler decodes field offset/width and extracts correctly every iteration. Fix: gate the dispatcher diagnostics behind `B2_JIT_DIAG` (default off). Verified jit-test 302/302, fail_equiv=0. **Current frontier:** post-video-init, gray screen; guest `0408795a` NuBus probe (`mac=f0000000`).
