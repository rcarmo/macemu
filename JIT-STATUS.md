...
**JIT optlev=2:** ⚠️ Full-JIT default. The historical `040ba0xx` late-ROM spin is **FIXED (2026-06-22)** — see below. With two fixes (lazy-cache-flush staleness + cache-tag aliasing) the all-native boot now clears the NuBus/Slot-Manager frontier and the `040026f8` aliasing frontier, advancing much further (DC ~590-766M) to the current frontier `0402e8d0`; desktop not yet reached.
