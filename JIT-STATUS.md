...
**JIT optlev=2:** ⚠️ Full-JIT default. The historical `040ba0xx` late-ROM spin is **FIXED (2026-06-22)** — see below. With three fixes (lazy-cache-flush staleness + cache-tag aliasing + dispatcher-diagnostic gating) the all-native boot clears the NuBus/Slot-Manager frontier and the `040026f8` aliasing frontier, and prints video-init markers (`VideoDriverOpen` → `SetEntries` → `SetGamma`). **It then still FREEZES at `0402e8d0`** — a slot-ROM `bfextu` blit loop — confirmed live under gdb on the default build (`pc_p=0402e8d0, D5=3, D6=16, A2=dd70`, stable across samples and over time). The loop completes exactly one iteration (`d5` 4→3, `d6` -16→16) then re-dispatches at `0402e8d0` without further progress. Tick-model-independent (sync and async both freeze; async freezes earlier). Desktop not reached.
