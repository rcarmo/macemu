...
placeholder screenshot manifest. This is the CI-safe mode for validating stories and reports.
A real driver should preserve the same operation names and can use a VNC library,
`vncdotool`, or a small Python/Node bridge.
...
- optional `report.html` / `*.pdf` generated by `tools/generate-pdf-report.mjs`

These artifacts are generated output. Keep them in ignored run directories such as
`BasiliskII/qa/artifacts/`, `qa/artifacts/`, or `/tmp/...`; commit only curated fixture
